...
flow meteres were used to observe the gas flow and evaporation of the
...
Fig.~\ref{fig:4kfilling} shows 5 filling cycles of the 4~K reservoir
...
\cite{bib:sweitz}. The level of helium in the 1~K pot is measured by a
...
data timestamps, ChargeL, ChargeS, PSD~(see
...
I developed a ROOT program that extracts the UCN cycle information for
each MIDAS run and creates a .root file.  The output file has one tree
as ``cycle information'' table. It consists of branches with the cycle
average, maximum, and minimum values for the main sensors that
...
light-guide to its corresponding photomultiplier tube~(PMT) outside
