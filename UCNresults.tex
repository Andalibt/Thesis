\chapter{UCN Production, Transport, and Detection\label{chap:UCNresult}}
...
\begin{figure}[h!]
  ...
  \caption{Number of UCN extracted from the source at different
    ...
    the eye. The simulations were performed with two values of B: B =
    0.008/s~(as shown with the top two lines) and B = 0.016/s~(as
    shown with the bottom two lines). The measured data is best
    matched with the simulations where B = 0.016/s where the helium
    gas is also taken into account.}
...
\begin{figure}[h!]
  ...
  \caption{Storage lifetime of UCN in the source at different
    superfluid helium temperatures~(filled circules).  The empty
    squares represent the simulations where the helium gas above the
    superfluid helium liquid is taken into account and the empty
    triangles represent the simulations where the helium gas is not
    included. The lines are interpolations of simulated data to guide
    the eye. The simulations were performed with two values of B: B =
    0.008/s~(the top two lines and B = 0.016/s. The measured data is
    best matched with the simulations where B = 0.016/s where the
    helium gas is also taken into account.  The lines are
