...
removed to keep the temperature of the superfluid helium constant. The
...
transfer in that region is not fully understood.  To check the
validity of theoretical models, the extrapolation to lower
...
causes the flow rate in the $^3$He pot to increase. After some time,
the temperature of the superfluid starts to settle and reach a new
equilibrium. Here this temperature is referred to as the
...
pressure dependence could be neglected. There models are the theory
...
The data that is shown in Table.~\ref{tab:heattest} is a collection of
cleaned data. These measurements showed that the heat input should not
be higher than the cryostat cooling power otherwise the temperature of
the superfluid helium would increase linearly without reaching an
equilibrium.

For a given heat input, the temperature difference between the base
and the saturated temperature for each temperature sensor is
...
as the raw data. However, the heat input for those data points should
be corrected since the heat input is consisted only of the added heat
from the heater tapes and the background heat is not taken into
account.

% For Monday or Tuesday July 9 or 10
% write about the JT and the other background heat
% describe the graphs for April and November tests
% Make a conclusion
% Add your heater test graphs and say that it is all bullshit
...
\begin{figure}[h!]
  ...
  \caption{The comparison between the April heat test data and the Van
    ...
    (included JT effect) and the red points show the raw data with the
    assumed 50 mW background heat input. }
